...
const LoginPage = () => {
  ...
  const handleLogin = (e) => {
    ...
    // For now, we'll just navigate to the respective province page based on the email
    const users = [
      { email: 'deliverMan@bookpoint.com', password: 'deliverManPassWord' },
    ];

    const user = users.find((u) => u.email === email && u.password === password);

    if (user) {
      navigate(`/ordersdelivery`, { state: { email } });
    } else {
...
<div className="w-full px-4 py-12 sm:px-6 sm:py-16 lg:w-1/2 lg:px-8 lg:py-24 bg-black" >
 ...
 <div className="mx-auto max-w-lg text-center">
   ...
   <h1 className="text-2xl text-yellow-300 font-bold sm:text-4xl"   style={{ fontFamily: 'Bona Nova'}}>Welcome, Delivery Team Member to Neth BookPoint!</h1>

   <p className="mt-4 text-yellow-600" style={{ fontFamily: 'Bona Nova'}}>
   We're glad to have you on our team and appreciate your dedication. Keep up the excellent work! Good luck in all your deliveries.

Login here to view your assigned tasks and continue delivering outstanding service.   </p>
...
<div className="relative h-70 w-full sm:h-96 lg:h-full lg:w-1/2">
 <img
   alt=""
   src="https://i.imgur.com/75m8gwH.jpeg"
